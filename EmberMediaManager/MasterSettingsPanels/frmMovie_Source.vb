...
#Region "Fields"
    ...
    Private _TmpSources As New Dictionary(Of Moviesource, State)
...
#Region "Dialog Methods"
    ...
    Public Sub New()
        ...
        ChildType = Enums.SettingsPanelType.MovieSource
        IsEnabled = True
        Image = Nothing
        ImageIndex = 5
        Order = 200
...
#Region "Interface Methodes"
    ...
    Public Sub SaveSettings() Implements Interfaces.ISettingsPanel.SaveSettings
        With Master.eSettings
            .MovieCleanDB = chkCleanLibraryAfterUpdate.Checked
            .GeneralDateAddedIgnoreNFO = chkDateAddedIgnoreNFO.Checked
            .GeneralDateTime = CType(cbDateAddedDateTime.SelectedItem, KeyValuePair(Of String, Enums.DateTimeStamp)).Value
            If Not String.IsNullOrEmpty(cbSourcesDefaultsLanguage.Text) Then
                .MovieGeneralLanguage = APIXML.ScraperLanguages.Languages.FirstOrDefault(Function(l) l.Description = cbSourcesDefaultsLanguage.Text).Abbreviation
            End If
            ...
            If Not String.IsNullOrEmpty(txtSkipLessThan.Text) AndAlso Integer.TryParse(txtSkipLessThan.Text, 0) Then
                .MovieSkipLessThan = Convert.ToInt32(txtSkipLessThan.Text)
            Else
                .MovieSkipLessThan = 0
            End If
            .MovieSortBeforeScan = chkSortBeforeScan.Checked
            '.TitleFiltersEnabled = chkTitleFiltersEnabled.Checked
            .MovieProperCase = chkTitleProperCase.Checked
            .GeneralSourceFromFolder = chkVideoSourceFromFolder.Checked
        End With
...
#Region "Methods"
    ...
    Public Sub Settings_Load()
        With Master.eSettings
            cbDateAddedDateTime.SelectedValue = .GeneralDateTime
            ...
            chkCleanLibraryAfterUpdate.Checked = .MovieCleanDB
            chkDateAddedIgnoreNFO.Checked = .GeneralDateAddedIgnoreNFO
            ...
            chkOverwriteNfo.Checked = .GeneralOverwriteNfo
            'chkResetNewBeforeDBUpdate.Checked = .ResetNewBeforeDBUpdate
            'chkResetNewOnExit.Checked = .ResetNewOnExit
            chkSortBeforeScan.Checked = .MovieSortBeforeScan
            'chkTitleFiltersEnabled.Checked = .TitleFiltersEnabled
            'chkTitleProperCase.Checked = .MovieProperCase 
            'chkVideoSourceFromFolder.Checked = .GeneralSourceFromFolder 
            'dgvTitleFilters.Enabled = .TitleFiltersEnabled
            'lblTitleFilters.Enabled = .TitleFiltersEnabled
            txtSkipLessThan.Text = .MovieSkipLessThan.ToString

            DataGridView_Fill_TitleFilters(.MovieFilterCustom)
        End With
        'With Master.eSettings.Movie.SourceSettings
        ...
        For Each source In Master.DB.LoadAll_Sources_Movie
    ...
    Private Sub LoadDefaults_TitleFilters() Handles btnTitleFilterDefaults.Click
        If MessageBox.Show(Master.eLang.GetString(840, "Are you sure you want to reset to the default filter list?"),
            ...
            DataGridView_Fill_TitleFilters(Master.eSettings.MovieFilterCustom.GetDefaults())
        ...
        For Each r As DataGridViewRow In dgvSources.Rows
            Select Case True
                ...
                Case CInt(r.Cells(0).Value) = State.[New]
                    '1 = new source
                    RaiseEvent NeedsDBUpdate_Movie(Master.DB.Save_Source_Movie(_TmpSources.Keys.FirstOrDefault(Function(f) f.IdSource = CLng(r.Cells(1).Value))))
                Case CInt(r.Cells(0).Value) = State.Edited
                    '2 = existing and edited source
                    Master.DB.Save_Source_Movie(_TmpSources.Keys.FirstOrDefault(Function(f) f.IdSource = CLng(r.Cells(1).Value)))
    ...
    Private Sub Save_TitleFilters()
        With Master.eSettings.MovieFilterCustom
            .Clear()
