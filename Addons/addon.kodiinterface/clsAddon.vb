...
#Region "Events"
    ...
    Public Event GenericEvent(ByVal eventType As Enums.AddonEventType, ByRef parameters As List(Of Object)) 'Implements Interfaces.IAddon.GenericEvent
    'Public Event AddonSettingsChanged() Implements Interfaces.IAddon.AddonSettingsChanged
    'Public Event AddonStateChanged(ByVal name As String, ByVal state As Boolean, ByVal diffOrder As Integer) Implements Interfaces.IAddon.AddonStateChanged
    'Public Event AddonNeedsRestart() Implements Interfaces.IAddon.AddonNeedsRestart
...
#Region "Properties"
    ...
    'ReadOnly Property Name() As String Implements Interfaces.IAddon.Name
    '    Get
    '        Return _Name
    '    End Get
    'End Property
    '
    'ReadOnly Property Version() As String Implements Interfaces.IAddon.Version
    '    Get
...
#Region "Methods"
    ...
    Public Function Run(ByRef dbElement As Database.DBElement, ByVal eventType As Enums.AddonEventType, ByVal parameters As List(Of Object)) As Interfaces.AddonResult Implements Interfaces.IAddon.Run
        If Not Master.isCL AndAlso (
                eventType = Enums.AddonEventType.Remove_Movie OrElse
                eventType = Enums.AddonEventType.Remove_TVEpisode OrElse
                eventType = Enums.AddonEventType.Remove_TVShow OrElse
                eventType = Enums.AddonEventType.Sync_Movie OrElse
                eventType = Enums.AddonEventType.Sync_MovieSet OrElse
                eventType = Enums.AddonEventType.Sync_TVEpisode OrElse
                eventType = Enums.AddonEventType.Sync_TVSeason OrElse
                eventType = Enums.AddonEventType.Sync_TVShow) Then
            'add job to tasklist and get everything done
            AddTask(New KodiTask With {.mType = eventType, .mDBElement = dbElement})
        ...
        Else
            ...
            Dim mDBElement As Database.DBElement = dbElement
            Dim tTask As Task(Of Boolean) = Task.Run(Function() DoCommandLine(eventType, mDBElement))
    ...
    Private Sub AddTask(ByRef nTask As KodiTask)
        ...
        If TasksDone Then
            RunTasks()
        Else
            TaskManager_Update_Status(lblTaskManagerStatus, String.Concat("Pending Tasks: ", (TaskList.Count + 1).ToString))
        End If
    End Sub
    ...
    Private Async Sub RunTasks()
        ...
        While TaskList.Count > 0
            TaskManager_Update_Status(lblTaskManagerStatus, String.Concat("Pending Tasks: ", TaskList.Count.ToString))
            TaskManager_Update_ProgressBar(tspTaskManager, ProgressBarStyle.Marquee)
        ...
        End While
        TasksDone = True
        TaskManager_Update_ProgressBar(tspTaskManager, ProgressBarStyle.Continuous)
        TaskManager_Update_Status(lblTaskManagerStatus, "No Pending Tasks")
        If Not getError Then
            Notifications.NewNotification(
                Notifications.Type.Information,
                Localisation.GetString(1, "Kodi Interface"),
                Localisation.GetString(2, "All Tasks Done")
                )
        Else
            Notifications.NewNotification(
                Notifications.Type.Error,
                Localisation.GetString(1, "Kodi Interface"),
                String.Format(Localisation.GetString(3, "One or more Task(s) failed.{0}Please check log for more informations."), Environment.NewLine)
                )
        End If
    End Sub
    ...
    ''' </remarks>
    Sub Init() Implements Interfaces.IAddon.Init
        LoadSettings()
    End Sub
    ''' <summary>
    ''' Load module settings
    ...
    Private Sub CreateContextMenu(ByRef tMenu As ToolStripMenuItem, ByVal tContentType As Enums.ContentType)
        ...
        If _SpecialSettings.Hosts IsNot Nothing AndAlso _SpecialSettings.Hosts.Count = 1 Then
            ...
            mnuHostSyncItem.Text = Localisation.GetString(9, "Sync")
            Select Case tContentType
                ...
                Case Enums.ContentType.TVSeason
                    AddHandler mnuHostSyncItem.Click, AddressOf Sync_TVSeason
                Case Enums.ContentType.TVShow
                    AddHandler mnuHostSyncItem.Click, AddressOf Sync_TVShow
            End Select
            ...
            If tContentType = Enums.ContentType.TVSeason OrElse tContentType = Enums.ContentType.TVShow Then
                ...
                mnuHostSyncFullItem.Text = Localisation.GetString(10, "Sync Full")
                Select Case tContentType
                    Case Enums.ContentType.TVSeason
                        AddHandler mnuHostSyncFullItem.Click, AddressOf Sync_Full_TVSeason
                    Case Enums.ContentType.TVShow
                        AddHandler mnuHostSyncFullItem.Click, AddressOf Sync_Full_TVShow
                End Select
            ...
            If tContentType = Enums.ContentType.Movie OrElse tContentType = Enums.ContentType.TVEpisode OrElse tContentType = Enums.ContentType.TVSeason OrElse tContentType = Enums.ContentType.TVShow Then
                If _SpecialSettings.GetWatchedState AndAlso Not String.IsNullOrEmpty(_SpecialSettings.GetWatchedStateHost) Then
                    ...
                    If mHost IsNot Nothing Then
                        ...
                        mnuHostGetPlaycount.Text = Localisation.GetString(11, "Get Watched-State")
                        Select Case tContentType
                            Case Enums.ContentType.Movie
            ...
            If tContentType = Enums.ContentType.Movie OrElse tContentType = Enums.ContentType.TVEpisode OrElse tContentType = Enums.ContentType.TVShow Then
                ...
                mnuHostRemoveItem.Text = Localisation.CommonWordsList.Remove
                Select Case tContentType
                    Case Enums.ContentType.Movie
                        AddHandler mnuHostRemoveItem.Click, AddressOf Remove_Movie
                    Case Enums.ContentType.TVEpisode
                        AddHandler mnuHostRemoveItem.Click, AddressOf Remove_TVEpisode
                    Case Enums.ContentType.TVShow
    ...
    Private Sub CreateToolsMenu(ByRef tMenu As ToolStripMenuItem)
        If _SpecialSettings.Hosts IsNot Nothing AndAlso _SpecialSettings.Hosts.Count = 1 Then
            ...
            mnuHostScanVideoLibrary.Text = Localisation.GetString(82, "Update Library")
            AddHandler mnuHostScanVideoLibrary.Click, AddressOf VideoLibrary_Update
            ...
            mnuHostCleanVideoLibrary.Text = Localisation.GetString(709, "Clean Database")
            AddHandler mnuHostCleanVideoLibrary.Click, AddressOf VideoLibrary_Clean
            ...
            If _SpecialSettings.GetWatchedState AndAlso Not String.IsNullOrEmpty(_SpecialSettings.GetWatchedStateHost) Then
                ...
                If mHost IsNot Nothing Then
                    ...
                    mnuHostGetPlaycount_Movies.Text = String.Format("{0} - {1}",
                                                                    Localisation.GetString(11, "Get Watched-State"),
                                                                    Localisation.CommonWordsList.Movies)
                    AddHandler mnuHostGetPlaycount_Movies.Click, AddressOf GetPlaycount_All_Movie
                    ...
                    AddHandler mnuHostGetPlaycount_TVEpisodes.Click, AddressOf GetPlaycount_All_TVEpisode
        ...
        ElseIf _SpecialSettings.Hosts IsNot Nothing AndAlso _SpecialSettings.Hosts.Count > 1 Then
            For Each kHost As Host In _SpecialSettings.Hosts
                ...
                mnuHostScanVideoLibrary.Text = Localisation.GetString(13, "Update Library")
                AddHandler mnuHostScanVideoLibrary.Click, AddressOf VideoLibrary_Update
                mnuHost.DropDownItems.Add(mnuHostScanVideoLibrary)
                Dim mnuHostCleanVideoLibrary As New ToolStripMenuItem
                mnuHostCleanVideoLibrary.Image = New Bitmap(My.Resources.menuClean)
                mnuHostCleanVideoLibrary.Tag = kHost
                mnuHostCleanVideoLibrary.Text = Localisation.GetString(14, "Clean Database")
                AddHandler mnuHostCleanVideoLibrary.Click, AddressOf VideoLibrary_Clean
    ...
    Private Sub PopulateMenus()
        ...
        Dim ts As ToolStrip = DirectCast(Addons.Instance.RuntimeObjects.MainToolStrip, ToolStrip)
    ...
    Sub Disable()
        ...
        'Task Manager
        Dim ts As ToolStrip = Addons.Instance.RuntimeObjects.MainToolStrip
    ...
    Public Sub InjectSettingsPanels() Implements Interfaces.IAddon.InjectSettingsPanels
        ...
        '_setup.UniqueName = _Name
        '_setup.Title = "Kodi Interface"
        '_setup.Type = Master.eLang.GetString(802, "Addons")
        '_setup.ImageIndex = If(_Enabled, 9, 10)
        '_setup.Order = 100
        '_setup.Panel = _setup.pnlSettings()
        '
    ...
    Public Sub SaveSettings(ByVal doDispose As Boolean) Implements Interfaces.IAddon.SaveSettings
        IsEnabled_Generic = _setup.chkEnabled.Checked
        ...
        If IsEnabled_Generic Then PopulateMenus()
        If doDispose Then
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub GetPlaycount_All_Movie(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Else
            Notifications.NewNotification(
                Notifications.Type.Error,
                Localisation.GetString(1, "Kodi Interface"),
                Localisation.GetString(12, "No Host configured"))
        End If
    End Sub
    ''' <summary>
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub GetPlaycount_All_TVEpisode(ByVal sender As Object, ByVal e As EventArgs)
        Dim Host As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If Host IsNot Nothing Then
            Dim _APIKodi As New Kodi.ApiKodi(Host)
            'add job to tasklist and get everything done
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub GetPlaycount_Movie(ByVal sender As Object, ByVal e As EventArgs)
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub GetPlaycount_TVEpisode(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
    ...
    ''' </remarks>
    Private Sub GetPlaycount_TVSeason(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub GetPlaycount_TVShow(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub Remove_Movie(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            For Each sRow As DataGridViewRow In Addons.Instance.RuntimeObjects.MediaListMovies.SelectedRows
                Dim ID As Long = Convert.ToInt64(sRow.Cells("idMovie").Value)
                Dim DBElement As Database.DBElement = Master.DB.Load_Movie(ID)
                'add job to tasklist and get everything done
                AddTask(New KodiTask With {.mDBElement = DBElement, .mHost = mHost, .mType = Enums.AddonEventType.Remove_Movie})
            Next
        Else
            Notifications.NewNotification(
                Notifications.Type.Error,
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub Remove_TVEpisode(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            For Each sRow As DataGridViewRow In Addons.Instance.RuntimeObjects.MediaListTVEpisodes.SelectedRows
                Dim ID As Long = Convert.ToInt64(sRow.Cells("idEpisode").Value)
                Dim DBElement As Database.DBElement = Master.DB.Load_TVEpisode(ID, True)
                'add job to tasklist and get everything done
                AddTask(New KodiTask With {.mDBElement = DBElement, .mHost = mHost, .mType = Enums.AddonEventType.Remove_TVEpisode})
            Next
        Else
            Notifications.NewNotification(
                Notifications.Type.Error,
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub Remove_TVShow(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            For Each sRow As DataGridViewRow In Addons.Instance.RuntimeObjects.MediaListTVShows.SelectedRows
                Dim ID As Long = Convert.ToInt64(sRow.Cells("idShow").Value)
                ...
                'add job to tasklist and get everything done
                AddTask(New KodiTask With {.mDBElement = DBElement, .mHost = mHost, .mType = Enums.AddonEventType.Remove_TVShow})
            Next
        Else
            Notifications.NewNotification(
                Notifications.Type.Error,
    ...
    ''' Update details of season in Kodi DB
    ''' </remarks>
    Private Sub Sync_Full_TVSeason(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            For Each sRow As DataGridViewRow In Addons.Instance.RuntimeObjects.MediaListTVSeasons.SelectedRows
                Dim ID As Long = Convert.ToInt64(sRow.Cells("idSeason").Value)
    ...
    ''' </remarks>
    Private Sub Sync_Full_TVShow(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            For Each sRow As DataGridViewRow In Addons.Instance.RuntimeObjects.MediaListTVShows.SelectedRows
                Dim ID As Long = Convert.ToInt64(sRow.Cells("idShow").Value)
    ...
    ''' </remarks>
    Private Sub Sync_Movie(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            For Each sRow As DataGridViewRow In Addons.Instance.RuntimeObjects.MediaListMovies.SelectedRows
                Dim ID As Long = Convert.ToInt64(sRow.Cells("idMovie").Value)
    ...
    ''' </remarks>
    Private Sub Sync_Movieset(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
    ...
    ''' </remarks>
    Private Sub Sync_TVEpisode(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            For Each sRow As DataGridViewRow In Addons.Instance.RuntimeObjects.MediaListTVEpisodes.SelectedRows
                Dim ID As Long = Convert.ToInt64(sRow.Cells("idEpisode").Value)
    ...
    ''' Update details of season in Kodi DB
    ''' </remarks>
    Private Sub Sync_TVSeason(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            For Each sRow As DataGridViewRow In Addons.Instance.RuntimeObjects.MediaListTVSeasons.SelectedRows
                Dim ID As Long = Convert.ToInt64(sRow.Cells("idSeason").Value)
    ...
    ''' Update details of tvshow in Kodi DB
    ''' </remarks>
    Private Sub Sync_TVShow(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            For Each sRow As DataGridViewRow In Addons.Instance.RuntimeObjects.MediaListTVShows.SelectedRows
                Dim ID As Long = Convert.ToInt64(sRow.Cells("idShow").Value)
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub VideoLibrary_Clean(ByVal sender As Object, ByVal e As EventArgs)
        Dim mHost As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If mHost IsNot Nothing Then
            Dim _APIKodi As New Kodi.ApiKodi(mHost)
            'add job to tasklist and get everything done
            AddTask(New KodiTask With {.mHost = mHost, .mInternalType = InternalType.VideoLibrary_Clean, .mType = Enums.AddonEventType.Task})
        Else
            Notifications.NewNotification(
                Notifications.Type.Error,
                Localisation.GetString(1, "Kodi Interface"),
    ...
    ''' <remarks>
    ''' </remarks>
    Private Sub VideoLibrary_Update(ByVal sender As Object, ByVal e As EventArgs)
        Dim Host As Host = DirectCast(DirectCast(sender, ToolStripMenuItem).Tag, Host)
        If Host IsNot Nothing Then
            Dim _APIKodi As New Kodi.ApiKodi(Host)
            'add job to tasklist and get everything done
    ...
    Public Sub TaskManager_Update_Status(control As ToolStripLabel, value As String)
        If control.Owner.InvokeRequired Then
            control.Owner.BeginInvoke(New Delegate_ChangeTaskManagerStatus(AddressOf TaskManager_Update_Status), New Object() {control, value})
        Else
            control.Text = value
        End If
    End Sub

    Public Sub TaskManager_Update_ProgressBar(control As ToolStripProgressBar, value As ProgressBarStyle)
        If control.Owner.InvokeRequired Then
            control.Owner.BeginInvoke(New Delegate_ChangeTaskManagerProgressBar(AddressOf TaskManager_Update_ProgressBar), New Object() {control, value})
        Else
            control.Style = value
        End If
    End Sub
...
#Region "Properties"
        ...
        <XmlIgnore>
        Public Property APIVersionInfo() As Kodi.ApiKodi.APIVersionInfo = New Kodi.ApiKodi.APIVersionInfo
