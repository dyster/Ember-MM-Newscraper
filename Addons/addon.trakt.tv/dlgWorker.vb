...
#Region "Dialog Methods"
    ...
    Public Sub New(ByRef traktApi As ApiTrakt, ByVal contentType As Enums.ContentType)
    ...
    Private Sub Setup()
        With Addon.Localisation
            ...
            btnStart.Text = .GetString(6, "Start Syncing")
...
#Region "Methods"
                            ...
                            If lstDBElement IsNot Nothing Then
                                ...
                                For Each tDBElement In lstDBElement.Where(Function(f) Not f.MainDetails.LastPlayed = strLastPlayed OrElse
                                    ...
                                    If tDBElement.IsOnline OrElse FileUtils.Common.CheckOnlineStatus(tDBElement, False) Then
                                        ...
                                        iItemsSynced += 1
                                        logger.Trace(String.Format("[TraktWorker] GetPlaycount_AllMovies: ""{0}"" | Synced to Ember", tDBElement.MainDetails.Title))
                                    Else
                                        logger.Warn(String.Format("[TraktWorker] GetPlaycount_AllMovies: ""{0}"" | NOT Synced to Ember, media was offline", tDBElement.MainDetails.Title))
                                    End If
                                Next
                            End If
                        Next
                                    ...
                                    For Each nWatchedEpisode In nWatchedSeason.Episodes
                                        ...
                                        For Each nTVShow In lstDBTVShow
                                            If nTVShow.IsOnline OrElse FileUtils.Common.CheckOnlineStatus(nTVShow, False) Then
                                                ...
                                                Next
                                            Else
                                                logger.Warn(String.Format("[TraktWorker] GetPlaycount_AllTVEpisodes: ""{0}"" | NOT Synced to Ember, tv show was offline", nTVShow.MainDetails.Title))
                                            End If
                                        Next
                                    Next
                                Next
    ...
    Private Sub bwGetWatchedState_ProgressChanged(ByVal sender As Object, ByVal e As System.ComponentModel.ProgressChangedEventArgs) Handles bwGetWatchedState.ProgressChanged
        Select Case e.ProgressPercentage
            Case 1
                ...
                lblOverallStateInfo.Text = Addon.Localisation.GetString(7, "Get watched media from Trakt.tv")
            ...
            Case 2
                ...
                lblOverallStateInfo.Text = Addon.Localisation.GetString(8, "Load database entries")
            ...
            Case 3
                ...
                lblOverallStateInfo.Text = Addon.Localisation.GetString(9, "Sync Ember with Trakt")
            ...
            Case 6
                'State 5: finished
                lblCurrentItemInfo.Text = String.Format("{0}: {1} {2}", Addon.Localisation.CommonWordsList.Done, CInt(e.UserState), Addon.Localisation.GetString(11, "Item(s) synced"))
                lblOverallStateInfo.Text = Addon.Localisation.CommonWordsList.Done
                prgbOverallState.Value = 4
                btnCancel.Text = Addon.Localisation.CommonWordsList.Close
            Case 7
                'no watched media found in Trakt
                lblCurrentItemInfo.Text = Addon.Localisation.GetString(10, "No watched media found on Trakt")
                lblOverallStateInfo.Text = Addon.Localisation.CommonWordsList.Done
    ...
    Private Sub bwGetWatchedState_RunWorkerCompleted(ByVal sender As Object, ByVal e As System.ComponentModel.RunWorkerCompletedEventArgs) Handles bwGetWatchedState.RunWorkerCompleted
        ...
        If Result.Cancelled Then
            lblCurrentItemInfo.Text = String.Empty
            lblOverallStateInfo.Text = Addon.Localisation.CommonWordsList.Cancelled
            prgbCurrentItem.Value = 0
            prgbOverallState.Value = 0
            btnStart.Enabled = True
            btnStart.Text = Addon.Localisation.GetString(6, "Start Syncing")
        End If
    End Sub
