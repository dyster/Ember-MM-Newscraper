...
#Region "Methods"
    ...
    Public Async Function CheckConnection() As Task(Of Boolean)
        ...
        Try
            ...
            If authorizationcheck.First AndAlso authorizationcheck.Second IsNot Nothing Then
                _client.Authorization = authorizationcheck.Second.Value
            ...
            Try
                ...
                Using dAuthorize As New frmAuthorize
                    If dAuthorize.ShowDialog(strActivationURL) = DialogResult.OK Then
                        Await _client.Authentication.GetAuthorizationAsync(dAuthorize.Result)
    ...
    Public Async Function RefreshAuthorization() As Task(Of Boolean)
        If _client.Authorization.IsRefreshPossible Then
            Dim bGetError As Boolean
            Try
                Dim authorization = Task.Run(Function() _client.Authentication.RefreshAuthorizationAsync)
                If authorization.Result.Value.IsValid Then
                    _client.Authorization = authorization.Result.Value
                    RaiseEvent NewTokenCreated()
                    Return True
                End If
            Catch ex As Exception
        ...
        If _client IsNot Nothing Then
            Select Case nContentType
                Case Enums.ContentType.Movie
                    ...
                    Else
                        logger.Info(String.Format("[GetID_Trakt] Could not scrape TraktID from trakt.tv! IMDB: {0} / TMDB: {1}", tDBElement.MainDetails.UniqueIDs.IMDbId, tDBElement.MainDetails.UniqueIDs.TMDbId.ToString))
                    End If
    ...
    Public Async Function GetInfo_Movie(ByVal uintTraktID As Task(Of UInteger), ByVal tFilteredOptions As Structures.ScrapeOptions) As Task(Of MediaContainers.Movie)
        ...
        If _client IsNot Nothing Then
            ...
            If tFilteredOptions.bMainRating Then
                ...
                If nGlobalRating IsNot Nothing AndAlso nGlobalRating.Value.Rating IsNot Nothing AndAlso nGlobalRating.Value.Votes IsNot Nothing Then
                    nMovie.Ratings.Add(New MediaContainers.RatingDetails With {
                                       .Max = 10,
                                       .Type = "trakt",
                                       .Value = Math.Round(nGlobalRating.Value.Rating.Value, 1),
                                       .Votes = CInt(nGlobalRating.Value.Votes)
                                       })
                End If
            End If
            ...
            If tFilteredOptions.bMainUserRating Then
                ...
                If bCheck.Result Then
                    Dim nPersonalRatedMovies = Await _client.Sync.GetRatingsAsync(TraktNet.Enums.TraktRatingsItemType.Movie)
                    If nPersonalRatedMovies IsNot Nothing AndAlso nPersonalRatedMovies.Count > 0 Then
                        Dim tMovie = nPersonalRatedMovies.FirstOrDefault(Function(f) f.MainDetails.Ids.Trakt = uintTraktID.Result)
    ...
    Public Async Function GetInfo_TVEpisode(ByVal uintTVShowTraktID As Task(Of UInteger),
                                            ...
                                            Optional ByVal tPersonalRatedTVEpisodes As IEnumerable(Of Objects.Get.Ratings.ITraktRatingsItem) = Nothing) As Task(Of MediaContainers.EpisodeDetails)
        If uintTVShowTraktID.Result = 0 Then Return Nothing

        Dim uintSeason As UInteger
        Dim uintEpisode As UInteger

        If Not UInteger.TryParse(intEpisode.ToString, uintEpisode) OrElse Not UInteger.TryParse(intSeason.ToString, uintSeason) Then
            Return Nothing
        End If
        ...
        If _client IsNot Nothing Then
            ...
            If tFilteredOptions.bEpisodeUserRating Then
                ...
                If nPersonalRatedTVEpisodes Is Nothing Then
                    Dim bCheck = CheckConnection()
                    If bCheck.Result Then
                        nPersonalRatedTVEpisodes = Await _client.Sync.GetRatingsAsync(TraktNet.Enums.TraktRatingsItemType.Episode)
                    End If
                End If
    ...
    Public Async Function GetInfo_TVShow(ByVal uintTraktID As Task(Of UInteger), ByVal FilteredOptions As Structures.ScrapeOptions, ByVal tScrapeModifiers As Structures.ScrapeModifiers, ByVal lstEpisodes As List(Of Database.DBElement)) As Task(Of MediaContainers.TVShow)
        ...
        If _client IsNot Nothing Then
            ...
            If FilteredOptions.bMainUserRating Then
                Dim bCheck = CheckConnection()
                If bCheck.Result Then
                    Dim nPersonalRatedTVShows = Await _client.Sync.GetRatingsAsync(TraktNet.Enums.TraktRatingsItemType.Show)
            ...
            If tScrapeModifiers.withEpisodes AndAlso lstEpisodes.Count > 0 AndAlso (FilteredOptions.bEpisodeRating OrElse FilteredOptions.bEpisodeUserRating) Then
                ...
                Dim nPersonalRatedTVEpisodes As IEnumerable(Of Objects.Get.Ratings.ITraktRatingsItem) = Nothing
                ...
                For Each nDBElement As Database.DBElement In lstEpisodes
                    Try
                        Dim nEpisode As MediaContainers.EpisodeDetails = Await GetInfo_TVEpisode(uintTraktID, nDBElement.MainDetails.Season, nDBElement.MainDetails.Episode, FilteredOptions, nPersonalRatedTVEpisodes)
