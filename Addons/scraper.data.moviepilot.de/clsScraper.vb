...
#Region "Methods"
    ...
    Public Function GetMovieInfo(ByVal originalTitle As String,
        ...
        Try
            Dim bIsScraperLanguage As Boolean = language.ToLower.StartsWith("de")

            Dim Result As New MediaContainers.Movie With {
                .Scrapersource = "Moviepilot.de"
            }


            Dim sURL As String = SearchMovie(originalTitle, year)
            'if theres no link with originaltitle, try with title
            If String.IsNullOrEmpty(sURL) AndAlso Not originalTitle = title Then
                sURL = SearchMovie(title, year)
            End If
